...
public class LocationManager extends CordovaPlugin implements BeaconConsumer {
    ...
    public void initialize(CordovaInterface cordova, CordovaWebView webView) {
		...
		startServiceIntent = new Intent(this.getApplicationContext(), BackgroundBeaconService.class);
    ...
    private void startMonitoringForRegion(final JSONObject arguments, final CallbackContext callbackContext) {
		...
		_handleCallSafely(callbackContext, new ILocationManagerCommand() {
			...
			public PluginResult run() {
				...
				Region region = null;
				try {
					// Write ibeacon identifier, uuid info into file
					FileOutputStream fileout = getApplicationContext().openFileOutput("ibeacon.txt", getApplicationContext().MODE_PRIVATE);
					OutputStreamWriter outputWriter = new OutputStreamWriter(fileout);
					
					String ibeaconInfo = arguments.getString("identifier") + "," + arguments.getString("uuid");
					
					outputWriter.write(ibeaconInfo);
					outputWriter.close();
					
					cordova.getActivity().startService(startServiceIntent);
					PluginResult result = new PluginResult(PluginResult.Status.OK);
					result.setKeepCallback(true);
					return result;
				} catch (Exception e) {
    ...
    private void stopMonitoringForRegion(final JSONObject arguments, final CallbackContext callbackContext) {
		...
		_handleCallSafely(callbackContext, new ILocationManagerCommand() {
			...
			public PluginResult run() {
				...
				try {
					cordova.getActivity().stopService(startServiceIntent);
